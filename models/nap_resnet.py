...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1000, zero_init_residual=False,
        ...
        self.maxpools = [nn.Identity(), nn.AdaptiveMaxPool2d(1), nn.AdaptiveMaxPool2d(2), nn.AdaptiveMaxPool2d(3), nn.AdaptiveMaxPool2d(4)]
    ...
    def _forward_impl(self, x, quantile=None):
        # See note [TorchScript super()]
        x = self.conv1(x)
        x = self.bn1(x)
        x = self.relu(x)
        x = self.maxpool(x)
        # immediate1 = torch.flatten(self.avgpools[3](x), 1)
        # # print(f"shape: {immediate1.shape[-1]} nonzero {(immediate1 > 0 ).sum()}")
        # x = self.layer1(x)
        # immediate2 = torch.flatten(self.avgpools[2](x), 1)
        # x = self.layer2(x)
        # immediate3 = torch.flatten(self.avgpools[1](x), 1)
        # x = self.layer3(x)
        ...
        # immediate1 = torch.flatten(self.maxpool_2(x), 1)
        x = self.avgpool(x)
        x = torch.flatten(x, 1)
        x = self.fc(x)
        if quantile[0] == 0:
        ...
        #         np.where(immediate4.cpu().numpy() > np.quantile(immediate4.cpu().numpy(), quantile[0]),
        #                  immediate4.cpu(), 0))
        #     immediate5 = torch.tensor(
        #         np.where(immediate5.cpu().numpy() > np.quantile(immediate5.cpu().numpy(), quantile[0]),
        #                  immediate5.cpu(), 0))
        #     immediate = torch.cat((immediate1, immediate2, immediate3), dim=1)
            # immediate = torch.cat((immediate1, immediate2, immediate4), dim=1)
            # return x, immediate, [immediate3.shape[-1], immediate1.shape[-1], immediate2.shape[-1]]
            # return x, immediate, [immediate1.shape[-1], immediate2.shape[-1], immediate4.shape[-1]]
        # return x, immediate, [immediate1.shape[-1], immediate2.shape[-1], immediate3.shape[-1]]
        return x, immediate, [immediate.shape[-1]]

    def forward(self, x):
        return self._forward_impl(x)[0]

    def forward_nap(self, x, quantile):
        return self._forward_impl(x, quantile)
