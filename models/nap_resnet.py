...
class ResNet(nn.Module):
    ...
    def _forward_impl(self, x, quantile=None):
        ...
        x = self.relu(x)
        x = self.maxpool(x)
        immediate1 = torch.flatten(self.avgpools[1](x), 1)
        # print(f"shape: {immediate1.shape[-1]} nonzero {(immediate1 > 0 ).sum()}")
        x = self.layer1(x)
        immediate2 = torch.flatten(self.avgpools[1](x), 1)
        x = self.layer2(x)
        ...
        if quantile:
            ...
            immediate5 = torch.tensor(
                np.where(immediate5.cpu().numpy() > np.quantile(immediate5.cpu().numpy(), quantile[4]),
                         immediate5.cpu(), 0))
            immediate = torch.cat((immediate5, immediate4, immediate2, immediate3), dim=1)
