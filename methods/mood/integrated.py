...
class MOOD(AbstractMethodInterface):
    def __init__(self, args):
        ...
        self.seed = 1
        self.model_name = ""
        self.workspace_dir = "workspace/mood"
    ...
    def _train_epoch(self):
        ...
        for in_set, out_set in zip(self.train_loader, self.unknown_loader):
            ...
            # forward
            output = self.base_model.forward_mood(data, softmax=False)
            ...
            for j in range(len(output)):
                if not loss:
                    loss = F.cross_entropy(output[j][:len(in_set[0])], target)
                else:
                    loss += F.cross_entropy(output[j][:len(in_set[0])], target)
                Ec_out = -torch.logsumexp(output[j][len(in_set[0]):], dim=1)
                Ec_in = -torch.logsumexp(output[j][:len(in_set[0])], dim=1)
